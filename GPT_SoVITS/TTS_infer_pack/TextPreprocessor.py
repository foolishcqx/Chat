...
class TextPreprocessor:
    ...
    #         result.append(res)
    #     return result
    def preprocess(self, text, lang: str, text_split_method: str, version: str = "v2") -> List[Dict]:
        print(f"############ {i18n('切分文本')} ############")
        
        # 如果 text 是 list，则不自动切分，直接用用户提供的分段
        if isinstance(text, list):
            texts = []
            for t in text:
                t = self.replace_consecutive_punctuation(t)
                texts.append(t)
        else:
            # 原来的逻辑：先清理标点，再自动切分
            text = self.replace_consecutive_punctuation(text)
        ...
        result = []
        print(f"############ {i18n('提取文本Bert特征')} ############")
        for seg in tqdm(texts):
            phones, bert_features, norm_text = self.segment_and_extract_feature_for_text(seg, lang, version)
            if phones is None or norm_text == "":
                continue
            res = {
                "phones": phones,
